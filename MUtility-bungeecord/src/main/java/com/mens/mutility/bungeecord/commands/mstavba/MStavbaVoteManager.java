package com.mens.mutility.bungeecord.commands.mstavba;
...
import com.mens.mutility.bungeecord.utils.*;
...
public class MStavbaVoteManager {
    ...
    public MStavbaVoteManager(MUtilityBungeeCord plugin) {
        this.plugin = plugin;
        db = plugin.getDb();
        ...
        timer.setOnRunning((sec, tt) -> {
            if(sec % 900 == 0) {
                if(!stillActive()) {
                    active = false;
                    endCompetition();
                    messageChannel.broadcastMessage("mens:stop-mstavba");
                    tt.cancel();
                }
            }
            if(sec % 3600 == 0) {
                ProxyServer.getInstance().getPlayers().forEach(this::createVoteLink);
            }
        });
        timer.startTimer(-1);
    }

    public void createVoteLink(ProxiedPlayer player) {
        if(active) {
    ...
    private int getPlayerVoteNumber(ProxiedPlayer player, int seasonId) {
        int votes = 0;
        try {
            if(!db.getCon().isValid(0)) {
                db.openConnection();
    ...
    private void deletePlayerKey(ProxiedPlayer player) {
        try {
            if(!db.getCon().isValid(0)) {
                db.openConnection();
            }
    ...
    private void insertPlayerKey(ProxiedPlayer player, String key) {
        try {
            if(!db.getCon().isValid(0)) {
                db.openConnection();
            }
    ...
    private void sendToDiscord(ResultSet rs, String seasonDesc) {
        try {
            ...
            while(rs.next()) {
                ...
                image = rs.getString(4);
                votes = rs.getInt(5);
                builder.addField(i + ". místo", playerManager.getUsername(userId) + " se stavbou " + buildingName, true);
                builder.addField("Počet hlasů", String.valueOf(votes), true);
            ...
            String channelName = plugin.getConfiguration().getString("Discord.Rooms.Infoline");
